;(function ($) {
    Drupal.behaviors.wweBullyMap = {
        attach: function (context, settings) {
            //console.log(settings);

            Modernizr.load({
                test: Modernizr.svg,
                yep : settings.wweBullyMap.dataPath + 'lib/d3.v2.min.js',
                nope: settings.wweBullyMap.dataPath + 'lib/r2d3.v2.min.js',
                complete: function() {
            ...
            var d3Map = {
                dataPath: settings.wweBullyMap.dataPath,
                ...
                JSONresults:null,
                d3Selection:null,
                lawContainer:{key:"Law Only", code:1, color: settings.wweBullyMap.lawColor},
                policyContainer:{key:"Policy Only", code:2, color: settings.wweBullyMap.policyColor},
                bothContainer:{key:"Law & Policy", code:3, color: settings.wweBullyMap.lawPolicyColor},
                highlightColor: settings.wweBullyMap.hiliteColor,
                pathColor: settings.wweBullyMap.pathColor,
                pathWidth:"1px",
                ...
                init: function() {
                    ...
                    var drawMap = that.d3Selection.append('svg')
                        .attr('id', 'mapSVG')
                        .attr('viewBox',"0 0 " + that.defaultWidth + " " + that.defaultHeight)
                        .attr('perserveAspectRatio',"xMinYMid");
                    that.svgElement = $('#mapSVG');
                    that.updateMapHeight();
//            Add if live resizing necessary
//            that.addWindowResizeHandler();
                    d3.json(that.dataPath + "data/states_final.json", function(collection) {
                ...
                updateMapHeight: function(){
                    this.svgElement.height(this.calculateHeight());
                },
                calculateHeight: function(){
                    return Math.round(this.svgElement.width()/d3Map.defaultRatio());
                },
                addWindowResizeHandler: function(){
                    var that = this;
                    $(window).on('resize',function(){
                        if(that.svgElement.height() == that.calculateHeight())
                            updateMapHeight();
                    }).trigger('resize');
                },
...
function getNumInDom(element, selector){
    return jQuery.inArray(element,jQuery(selector));
}
