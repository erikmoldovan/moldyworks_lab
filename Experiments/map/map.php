 ...
 */
    ...
    foreach ($map['features'] as $value) {
        ...
        foreach ($output as $state_key => $state_values) {
            if (strpos($state_values['url'], $current_state) !== FALSE) {
                $final["features"][$state_key] = $state_values + $value;
                continue;
            }
        }
    }

    $fp = fopen('states_draft.json', 'w');
